...
class ImageEnhancement:
    def __init__(self, img, m, n, gamma, colored):
        if colored:  # in case of colored image we need to separate each color layer from each other.
            ...
            self.e_layer_b = translate(layer_b, 0, 255, -1, 1)
            self.e_layer_g = translate(layer_g, 0, 255, -1, 1)
            self.e_layer_r = translate(layer_r, 0, 255, -1, 1)
            self.img = real_scalar_multiplication(1 / 3, add(add(self.e_layer_b, self.e_layer_g), self.e_layer_r))

        else:  # in case of gray scale image; all the image consider as one layer.
            self.img = translate(img, 0, 255, -1, 1)
    ...
    def _fuzzy_mean(self, w, i, j):  # we use the mean values to enhance the brightness --> where tao = -1 * mean
        ...
        for k in range(self.height):
            for l in range(self.width):
                mean = add(mean, real_scalar_multiplication(w[i, j, l, k], self.img[k, l]))
    ...
    def _fuzzy_variance(self, w, i, j):  # we use the variance values to enhance the contrast --> where lambda = sqrt(
        ...
        for k in range(self.height):
            for l in range(self.width):
                variance += w[i, j, l, k] * np.power(norm(sub(self.img[k, l], self.means[i, j])), 2)
        variance /= self._card(w[i, j])
        return variance

    def _lambda_(self, i, j):
        return np.sqrt(1 / 3) / (np.sqrt(self.variances[i, j]) + eps)
    ...
    def _window_enh(self, i, j, layer):
        ...
        return real_scalar_multiplication(self.lambdas[i, j], sub(layer, self.taos[i, j]))
    ...
    def _apply_enhancement(self, layer):
        ...
        for i in range(self.m):
            for j in range(self.n):
                ...
                for k in range(self.width):
                    for l in range(self.height):
                        new_image[l, k] = add(new_image[l, k], real_scalar_multiplication(w1[l, k], win[l, k]))
    ...
    def _one_layer_enhancement(self, layer):  # gray scale images
        ...
        res_image = translate(e_layer, -1, 1, 0, 255)
