...
class Cropper:
    ...
    def prepare(self, imgs, masks, corners, sizes):
        if self.do_crop:
            # print('123242')
            non_crop_image, mask = self.estimate_panorama_mask(imgs, masks, corners, sizes)
    ...
    def estimate_panorama_mask(imgs, masks, corners, sizes):
        non_crop_image, mask = Blender.create_panorama(imgs, masks, corners, sizes)
        # import cv2
        # cv2.imshow('', non_crop_image[:, :, ::-1])
        # cv2.waitKey(0)
        return non_crop_image, mask
    ...
    def estimate_largest_interior_rectangle(self, mask):
        ...
        a = largestinteriorrectangle.lir(mask > 0, contour)
        # print(a)
        a = Rectangle(*a)
